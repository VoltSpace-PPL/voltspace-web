...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <div class="lg:col-span-4 rounded-2xl p-5 flex flex-col" style="background:#161e2d; border:1px solid #1e2d45;">
        ...
        <div class="mt-6 flex flex-col gap-2 border-t pt-4" style="border-color:#1e2d45;">
            <div class="flex items-center gap-2 text-[12px] text-slate-400">
                <div class="w-3 h-3 rounded bg-red-500/20 border border-red-500/40"></div> Terpakai (Booked / In Use)
            </div>
            <div class="flex items-center gap-2 text-[12px] text-slate-400">
                <div class="w-3 h-3 rounded bg-[#00d4aa]/20 border border-[#00d4aa]/40"></div> Selected Date
            </div>
            <div class="flex items-center gap-2 text-[12px] text-slate-400">
                <div class="w-3 h-3 rounded bg-slate-700 border border-slate-600"></div> Hari Ini
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-8 rounded-2xl p-5" style="background:#161e2d; border:1px solid #1e2d45;">
        <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
            ...
            <div class="flex items-center gap-4 text-[12px]">
                <div class="flex items-center gap-1.5 text-slate-400">
                    <div class="w-3 h-3 rounded bg-emerald-500/10 border border-emerald-500/30"></div> Tersedia
                </div>
                <div class="flex items-center gap-1.5 text-slate-400">
                    <div class="w-3 h-3 rounded bg-red-500/10 border border-red-500/30"></div> Terpakai
                </div>
            </div>
        </div>

        <div id="timeline-grid" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <!-- Populated by JS -->
        </div>
        ...
        <div id="timeline-loading" class="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <span class="text-slate-500 text-[13px]">Memuat jadwal...</span>
        </div>
    </div>
</div>
@endsection
    ...
    function renderTimeline(dateStr) {
        ...
        grid.innerHTML = hours.map(h => {
            ...
            if (isOccupied) {
                ...
                cls = 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 cursor-pointer';
                label = 'Tersedia';
                canBook = true;
            }

            return `
            <div class="rounded-xl p-4 border ${cls} transition-all relative group flex flex-col justify-center"
                 ${canBook ? `onclick="window.location.href='/student/bookings/create?ruangan_id=${roomId}&selected_date=${dateStr}&start_time=${slotStart}&end_time=${slotEnd}'"` : ''}>
                <p class="text-[16px] font-bold mb-1">${timeLabel}</p>
                <p class="text-[14px] font-semibold">${label}</p>
                ${subText ? `<p class="text-[12px] mt-1 opacity-80 truncate">${subText}</p>` : ''}
                
                ${canBook ? `
                <div class="absolute inset-0 flex items-center justify-center bg-emerald-500 text-[#0b1120] font-bold text-[14px] rounded-xl opacity-0 group-hover:opacity-100 transition-opacity">
                    📋 Buat Booking
                </div>` : ''}
            </div>`;
        }).join('');
